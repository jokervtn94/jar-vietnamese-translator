...
def test_diagnostics_workspace_is_full_width_page_without_importing_qt():
    ...
    for text in (source, portable):
        ...
        assert 'QPushButton("Diagnostics")' in text
        assert 'QPushButton("← Quay lại dịch")' in text
        assert 'stack.setObjectName("MainWorkspaceStack")' in text
        assert 'diagnostics_page.setObjectName("DiagnosticsPage")' in text
        ...
        assert 'stack.addWidget(translation_page)' in text
        assert 'stack.addWidget(diagnostics_page)' in text
        assert 'header_layout.insertWidget(max(0, build_index), diagnostics_btn, 0)' in text
